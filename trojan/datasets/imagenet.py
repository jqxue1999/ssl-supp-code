...
MEAN = const.IMAGENET_MEAN
STD = const.IMAGENET_STD
...
class FileListDataset(data.Dataset):
    def __init__(self, path_to_txt_file, t_0, n_0=1, t_1=None, n_1=0, n_2=0, return_tuple=False, one_image_path=None):
        ...
        self.base_transform = T.Compose(
            [T.Resize(224), T.CenterCrop(224), T.ToTensor(), T.Normalize(mean=MEAN, std=STD)])
    ...
    def __getitem__(self, idx):
        ...
        if self.t_1 is not None and self.n_2 != 0:
            for i in range(self.n_2):
                if self.one_image_path is not None:
...
class ImageNet(BaseDataset):
    def ds_train(self):
        ...
        return FileListDataset(path_to_txt_file=self.aug_cfg.train_file_path, t_0=t_0, n_0=self.aug_cfg.n_0, t_1=t_1,
                               ...
                               one_image_path=self.aug_cfg.one_image_path)
    ...
    def ds_test(self):
        setup_seed(127)
        aug_base = T.Compose([T.Resize(256), T.CenterCrop(224), T.ToTensor()])
        t = TriggerT(base_transform=aug_base, mean=MEAN, std=STD)
